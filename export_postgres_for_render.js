...
async function exportPostgresForRender() {
  try {
    ...
    // Escapar caminhos com espaços
    const schemaPath = `"${path.join(EXPORT_DIR, 'schema.sql')}"`;
    const dataPath = `"${path.join(EXPORT_DIR, 'data.sql')}"`;
    const dumpPath = `"${path.join(EXPORT_DIR, 'render_import.dump')}"`;
    
    // Exportar schema (estrutura do banco)
    console.log('📊 Exportando schema...');
    await execPromise(`pg_dump --host=${DB_CONFIG.host} --port=${DB_CONFIG.port} --username=${DB_CONFIG.user} --dbname=${DB_CONFIG.database} --schema-only --no-owner --no-privileges --file=${schemaPath}`);
    
    // Exportar dados
    console.log('📋 Exportando dados...');
    await execPromise(`pg_dump --host=${DB_CONFIG.host} --port=${DB_CONFIG.port} --username=${DB_CONFIG.user} --dbname=${DB_CONFIG.database} --data-only --no-owner --no-privileges --file=${dataPath}`);
    
    // Criar dump completo
    console.log('📦 Criando dump completo...');
    await execPromise(`pg_dump --host=${DB_CONFIG.host} --port=${DB_CONFIG.port} --username=${DB_CONFIG.user} --dbname=${DB_CONFIG.database} --no-owner --no-privileges --format=custom --file=${dumpPath}`);
